...
class Action_control(object): 
           ...
           #Serial GPIO 
           def Serial_DC_motordriver(self,mcu_number,number,pin_number,motor_name): # Getting the motor name and the speed of the motor 
              ...
              if mcu_number == "STM32F103C8TX": # Getting the microcontroller series
                  ...
                  exec("motorl_"+str(number) +" = motor_name.get_pin('d:"+str(pin_number[0])+":p')")
                  exec("motorr_"+str(number) +" = motor_name.get_pin('d:"+str(pin_number[1])+":p')")
              ...
              if mcu_number == "STM32F303K8TX":
                  ...
                  exec("if gpiol"+" == 0 and gpior"+"== 1:"+"\n\t"+"motorl_"+str(number)+".write(0))"+"\n\t"+"motorr_"+str(number)+".write("+str(speed)+")") 
                  exec("if gpiol"+" == 0 and gpior"+"== 0:"+"\n\t"+"motorl_"+str(number)+".write(0)"+"\n\t"+"motorr_"+str(number)+".write(0)") 
           def Serial_DC_pins_drive(self,mcu_number,number,speed,gpiol,gpior):
              if mcu_number == "STM32F103C8TX": # Getting the microcontroller series
                  print("Selected mcu: ",mcu_number) 
                  exec("if gpiol"+" == 1 and gpior"+"== 0:"+"\n\t"+"motorl_"+str(number)+".write("+str(speed)+")"+"\n\t"+"motorr_"+str(number)+".write(0)") 
                  exec("if gpiol"+" == 0 and gpior"+"== 1:"+"\n\t"+"motorl_"+str(number)+".write(0)"+"\n\t"+"motorr_"+str(number)+".write("+str(speed)+")") 
...
class NLP_language_trans(object): 
           ...
           def Language_translator(self,text,ip,port,lang):
                   translation = translator.translate(text,lang) # Getting the translation from text input
...
def Create_serial_motor(mcu_number,number,pin_number,motor_name,speed,gpiol,gpior):
        ...
        motor_node.Serial_DC_motordriver(mcu_number,number,pin_number,motor_name,speed,gpiol,gpior)
